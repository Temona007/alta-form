...
import base      from "./API/airtable.js";
  ...
  // Check if all required fields in the current step are filled
    ...
    if (allFilled) {
      if (currentStep === totalSteps - 1) {
        // Final step: Submit to Airtable
        setShowConfirmation(true);
        let nameFieldString = '';
        Object.entries(formData).forEach(([key, value]) => {
          if (value && value !== '') {
            const displayValue = typeof value === 'object' && value instanceof File ? value.name : value;
            nameFieldString += `${fieldLabels[key] || key}: ${displayValue}\n`;
          }
        });
        const airtableFields = {
          userInfo: nameFieldString.trim()
        };
        console.log('Submitting to Airtable table "userData":', airtableFields);
        base('userData').create([
          ...
          if (err) {
            console.log('Airtable submission error:', err);
            return;
          }
          records.forEach(function (record) {
            console.log('Airtable record created:', record.getId());
          });
        });
      } else {
        // Move to next step
